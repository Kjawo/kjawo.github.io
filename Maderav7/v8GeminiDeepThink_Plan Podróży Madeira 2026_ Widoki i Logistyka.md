...
- **Zdrowie, dokumenty, finanse:** polisa, EKUZ, assistance, kopie offline, druga karta, gotówka, procedura utraty telefonu, `5 l` wody w aucie.
...
Ta sekcja jest wersją terenową na telefon: jeśli jest presja czasu, decyzję podejmujesz z tej listy, nie przez czytanie całego dokumentu.

- **Auto GO:** umowa zgadza się z rezerwacją, karta działa na depozyt, zdjęcia auta zrobione, paliwo i zwrot jasne, brak presji na szutry. **NO-GO:** odmowa karty/depozytu, niejasny wkład własny, brak zgody na after-hours albo brak zdjęć auta - najpierw rozwiązać auto, potem plan.
...
- **Zmęczony kierowca:** koniec ambicji na dany dzień. Najbliższa baza, kawa/jedzenie, przerwa, ewentualnie powrót bez zachodu.
...
- **GO / NO-GO PR1:** GO tylko gdy IFCN/SIMplifica pokazuje dostępny odcinek, QR jest offline, brak ostrzeżenia pogodowego, widoczność jest realna i obie osoby spały minimum `7 h`. NO-GO oznacza przeniesienie według macierzy bramek, nie "zobaczymy na miejscu".
- **Zakaz:** nie wychodzić poza oficjalnie dostępny odcinek PR1, nawet jeśli inni idą dalej.
- **Skip if tired:** forsowanie dłuższego odcinka PR1 przy mgle, silnym wietrze, słabej widoczności albo po krótkim śnie.
...
- **Plan B:** zamiana z dniem wschodnim zgodnie z macierzą PR1/PR8 albo południe low-drive.
...
- **Plan B:** start dzienny, same wschodnie viewpointy + Machico / Canical; PR8 skrócić bez żalu.
...
- **Skip if tired:** dodatkowe objazdy w środku dnia, dokładanie losowych przystanków kosztem zachodu albo Ponta do Pargo sunset przy zmęczonym kierowcy.
...
- **Low energy:** Ponta da Ladeira + Garganta Funda; Ponta do Pargo sunset tylko jeśli kierowca jest świeży. Wysiłek: `very low`.
...
- **Sprawdzić wcześniej:** politykę paliwową, dokumenty, czas dojazdu do lotniska, warunki zdania auta, wymagany bufor zwrotu i bezpieczeństwo krótkiego ostatniego stopu.
- **Parking:** krótki stop tylko tam, gdzie nie musisz ciągnąć walizek daleko i gdzie bagaż nie zostaje eksponowany. Jeśli bagaż jest widoczny w aucie, stop odpada.
- **Dojście od parkingu:** orientacyjnie `2-10 min`.
...
### 5.0 Hierarchia źródeł

W razie sprzeczności wygrywa źródło bliżej operatora i transakcji:

1. **Operator / sprzedawca / aplikacja transakcyjna** - wypożyczalnia, Wizz, operator cable car, Carreiros, SIMplifica przy płatności.
2. **Visit Madeira PT / SIMplifica / IFCN** - dla szlaków, statusów i opłat publicznych.
3. **Visit Madeira EN** - dobre do logistyki i opisów, ale przy różnicy z PT/IFCN traktować jako niższy priorytet.
4. **Źródła wtórne** - tylko orientacja; nie podnoszą statusu do `Potwierdzone`.
...
- **Carreiros do Monte:** `35,00 EUR` za 2 osoby; `09:00-18:00`; kupowane na miejscu; miej gotówkę `35 EUR` + zapas; karta `[DO POTWIERDZENIA]`; możliwe wcześniejsze zamknięcie przy kolejce/pogodzie.
...
- **Zasada:** nie opłacaj alternatywnych PR "na wszelki wypadek". Nie zakładaj elastycznego rebookingu, jeśli oficjalne zamknięcie nie daje do tego podstawy.
...
Kuchnie poza Funchal często zamykają się w oknie `15:00-18:30`; konkretne miejsca sprawdź w dniu użycia. Jeśli przekroczysz to okno w górach, lądujesz w supermarkecie.
...
| Dzień | Punkt jedzenia B | Sklep B | Limit krążenia za parkingiem | Punkt odwrotu |
| --- | --- | --- | --- | --- |
...
- **Woda:** `5 l` wody w aucie jako stały zapas, uzupełniany po każdym dniu terenowym.
...
- praca cable cars,
- godziny Monte / cable car / Carreiros oraz płatność na miejscu, jeśli backup miejski ma wejść do gry,
- polityka paliwowa i zwrot auta, depozyt, wkład własny, szyby/opony/podwozie, zdjęcia auta,
- screenshot Wizz z aplikacji na `T-7`, `T-3`, `T-0`,
